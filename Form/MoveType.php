...
class MoveType extends AbstractType
{
	...
	public function buildForm(FormBuilderInterface $builder, array $options)
    {
		...
		$public = [
			'Non' => '',
			'Oui' => $this->kernel->getProjectDir().'/public/uploads/'
		];
        ...
        $builder
            ->add('tree', ChoiceType::class, [
                ...
                'choices' => $tree,
				'required' => false,
				'help' => 'Si oui, choisissez vers quel dossier de cet espace vous souhaitez déplacer l\'élément et laissez les autres champs sur "Non".'
            ])
        ...
        if($view !== "public") {
        	$builder
				->add('public', ChoiceType::class, [
					...
					'choices' => $public,
					'required' => false,
					'help' => 'Si oui, ne sélectionnez rien dans les autres champs. L\'élément sera déplacé à la racine de l\'espace cible. Sa visibilité sera probablement modifiée.'
				])
			;
		}
        ...
        if(count($privateSpaces) > 1) {
			$builder
				->add('private_space', ChoiceType::class, [
					'label' => 'Déplacer vers un dossier partagé ?',
					'choices' => $privateSpaces,
					'required' => false,
					'help' => 'Si oui, choisissez l\'espace cible et ne sélectionnez rien dans les autres champs. L\'élément sera déplacé à la racine de l\'espace cible. Sa visibilité sera probablement modifiée.',
				])
			;
		}
