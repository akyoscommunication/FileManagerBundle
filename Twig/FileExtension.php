...
class FileExtension extends AbstractExtension
{
    ...
    public function renderFileManager($value, $height = null, $width = null)
    {
        ...
        if (preg_match($ytPattern, $value)) {
            ...
            $result = '<img class="lazy-load not-loaded aky-img" src="data:image/svg+xml,%3Csvg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 '.($height ? $height : '100%').' '.($width ? $width : '100%').'"%3E%3C/svg%3E" data-src="'.$value.'" alt=""/>';
            ...
            if(explode( "/", mime_content_type(__DIR__.'/../../../public'.$file->getFile()))[0] == 'image' ){
                $result = '<img class="lazy-load not-loaded aky-img" src="data:image/svg+xml,%3Csvg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 '.($height ? $height : '100%').' '.($width ? $width : '100%').'"%3E%3C/svg%3E" data-src="'.$file->getFile().'" alt="'.$file->getAlt().'"/>';
            }else{
